document.addEventListener('DOMContentLoaded', function () {
    ...
    const saladStepData = [
        {
            ...
            subtitle: "处理食材",
            subSteps: [
                { name: "牛油果", steps: ["对半切开", "去除中间果核", "用勺子挖出果肉", "把果肉切成一手指盖的长宽大小"] },
        ...
        },
        {
            ...
            subtitle: "处理料汁",
            subSteps: [
                { name: "料汁", steps: ["取一个柠檬", "倒入一点橄榄油（大约一个瓶盖）", "挤一片柠檬汁", "加入四分之一勺盐和适量黑胡椒"] }
            ]
        },
        {
            name: "步骤3",
            subtitle: "混合食材",
            subSteps: [
                { name: "混合沙拉", steps: ["将牛油果、番茄、洋葱放入大碗", "倒入调好的料汁", "轻轻搅拌，确保均匀混合上酱汁"] }
            ]
        }
    ];
    ...
    // 将API数据转换为stepData格式
    function convertApiDataToStepData(apiData) {
        if (!apiData || !apiData.steps) return null;
        
        return apiData.steps.map((step, index) => {
